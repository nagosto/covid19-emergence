# Load packages
packages <- c("lubridate", "plotly", "RColorBrewer", "shiny", "tidyverse", "viridis")
for (i in packages) {
...
# Covid19 data
confirmed <- read.csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv", check.names = FALSE, stringsAsFactors = FALSE)
recovered <- read.csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_recovered_global.csv", check.names = FALSE, stringsAsFactors = FALSE)
deaths <- read.csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv", check.names = FALSE, stringsAsFactors = FALSE)

# US Covid19
confirmed_us <- read.csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_US.csv", check.names = FALSE, stringsAsFactors = FALSE)
deaths_us <- read.csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_US.csv", check.names = FALSE, stringsAsFactors = FALSE)

# World population data
world_pop <- read.csv("pop_per_country.csv", check.names = FALSE, stringsAsFactors = FALSE)
...
confirmed_us <- rename(confirmed_us, COUNTRY = "Country_Region", Long = "Long_", CODE = "iso3")
...
# Rename US 
confirmed[confirmed$COUNTRY == "US", ]$COUNTRY = "United States"

# Recode Puerto Rico as a country in US datasets
...
# Rename covid data sets country column so that it matches country_codes column name
...
# Eliminate unwanted variables in US data
confirmed_us <- confirmed_us[,-c(1:2,4:6)]
deaths_us <- deaths_us[,-c(1:2,4:6)]

# Add row to global datasets regarding PR
...
# Add US states abbreviations to data
state_codes <- data.frame(Province_State = state.name, Abb = state.abb, stringsAsFactors = FALSE)
deaths[which(deaths$COUNTRY=="US"),1]="United States"
confirmed_us <- confirmed_us %>% group_by(Province_State) %>% summarise_if(., is.numeric, funs(sum))
confirmed_us <- right_join(state_codes, confirmed_us, by = "Province_State")
deaths_us <- deaths_us %>% group_by(Province_State) %>% summarise_if(., is.numeric, funs(sum))
deaths_us <- right_join(state_codes, deaths_us, by = "Province_State")

# Add country codes to datasets 
...
# For UI
cols <- c("Blues","BuGn","BuPu","GnBu","Greens","Greys","Oranges","OrRd","PuBu","PuBuGn","PuRd","Purples","RdPu","Reds","YlGn","YlGnBu","YlOrBr","YlOrRd")
...
server <- function(input, output) {
  ...
  # US map with daily cases of selected country dataset
  output$usa <- renderPlotly({
    req(input$date)
    # Work with selected dataset
  ...
  # World map with daily cases of selectedcountry dataset
  output$world1 <- renderPlotly({
    req(input$view)
    date <- gsub('\\b0+','',format(input$date,'%m/%d/%20'))
    l <- list(color = toRGB("grey"), width = 0.5)
    g <- list(
  ...
  output$scatterp <- renderPlotly({
    ...
    for(trace in colNames){
      p <- p %>% add_trace(data= df2, x = ~date, y = as.formula(paste0("~`", trace, "`")), name = trace)
      p <- p %>% layout(xaxis = list(title ="Day", categoryarray = ~df2[,1], categoryorder = "array"),
                          yaxis= list(title = input$view2, categoryarray = ~trace, categoryorder = "array")) 
    }
    p
  })
  ...
  output$world2 <- renderPlotly({
    ...
    mapdata2[,7:NCOL(mapdata2)] %>% replace(., is.na(.), 0) ->  mapdata2[,7:NCOL(mapdata2)]
    mapworld2 <- plot_geo(mapdata2)
    mapworld2 <- mapworld2 %>% add_trace(
      z = ~ mapdata2[, which(names(mapdata2) == date)],
      color = "viridis",
      locations = ~ CODE,
      marker = list(line = l),
      hoverinfo = "text",
      text = ~ifelse(mapdata2[, which(names(mapdata2) == date)] == 0, COUNTRY, paste(COUNTRY, mapdata2[, which(names(mapdata2) == date)]))
    )
